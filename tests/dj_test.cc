...
TEST(ArgvFilteringTest, CheckForbiddenTokensRejected) {
  ...
  std::vector<std::pair<std::vector<std::string>, std::string>> testData = {
      {{"hmcdj", "track.yaml", "--grid", "4.4.4.4"}, "--grid"},
      {{"hmcdj", "track.yaml", "--StartingType", "HotStart"}, "--StartingType"},
      {{"hmcdj", "track.yaml", "--StartingTrajectory", "10"},
       "--StartingTrajectory"},
      {{"hmcdj", "track.yaml", "--Thermalizations", "10"}, "--Thermalizations"},
...
TEST(ArgvFilteringTest, CheckForbiddenTokensRejectedAfterAllowed) {
   ...
   */
  std::vector<std::string> hmcdjArgv = {"hmcdj",   "track.yaml",     "--mpi",
                                        "1.1.1.1", "--Trajectories", "100"};
...
TEST(ArgvFilteringTest, CheckForbiddenTokensRejectedInInvalidGridLine) {
   ...
   * options to Grid. */
  std::vector<std::string> hmcdjArgv = {"hmcdj", "track.yaml", "--Trajectories",
                                        "--grid"};
  testDeathGetGridArgv(hmcdjArgv, defaultGrid, "--Trajectories");
}
...
TEST(ArgFilteringTest, CheckMinimalCommandLine) {
  ...
  std::vector<std::string> hmcdjArgv = {"hmcdj", "track.yaml"};
  std::vector<std::string> expectedArgv = {"hmcdj", "--grid", "4.4.4.4"};
...
TEST(ArgFilteringTest, CheckCommandLineWithMPI) {
  ...
  std::vector<std::string> hmcdjArgv = {"hmcdj", "track.yaml", "--mpi",
                                        "1.1.1.1"};
  std::vector<std::string> expectedArgv = {"hmcdj", "--mpi", "1.1.1.1",
                                           "--grid", "4.4.4.4"};
  auto updatedArgv = testGetGridArgv(hmcdjArgv, defaultGrid);
  EXPECT_EQ(updatedArgv, expectedArgv);
}
